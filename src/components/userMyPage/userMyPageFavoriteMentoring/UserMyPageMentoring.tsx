...
const UserMyPageMentoring = ({ mentorItem }: IProps) => {
	...
	const replaceHandler = () => {
		const replaceAmount = mentorItem.amount.toLocaleString();
		const replaceHeadCount = mentorItem.amount.toLocaleString();

		const startDate = new Date(mentorItem.startDate);
		const endDate = new Date(mentorItem.endDate);
	...
	const getCategoryNameHandler = () => {
		for (let key in categories) {
			categories[key].find((category) => {
				if (category.key === mentorItem.category) {
