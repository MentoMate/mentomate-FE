...
const useUrl = (type: string, id: string | null = null) => {
	...
	if (type === "mentoring") {
		INITIAL_URL =
			"/mentoring/search?sortBy=latest&searchCategory=default&page=1&pageSize=16";
	}
	if (type === "mentor") {
		INITIAL_URL =
			"/mentor/search?sortBy=latest&searchCategory=default&page=1&pageSize=16";
	}
	if (type === "post") {
		INITIAL_URL =
	...
	const transformationUrl = () => {
		...
		let category =
			...
			selectedSearchCriteria.category === "all"
				? "default"
				: selectedSearchCriteria.category;
